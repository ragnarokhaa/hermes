...
function authenticityFromConfidence(confidence) {
  const parsed = Number.parseInt(String(confidence || "").replace("%", ""), 10);
  return Number.isFinite(parsed) ? parsed : 0;
}

function classifyAuthenticity(score) {
  if (score < 40) {
    return { status: "false", verdictLabel: "Fake", verdictTitle: "Fake" };
  }

  if (score > 70) {
    return { status: "true", verdictLabel: "True", verdictTitle: "True" };
  }
...
export function normalizeHermesResult({ query, speaker = "", type = "video", hermesContent }) {
  ...
  return {
    ...
    confidence,
    authenticity: authenticityFromConfidence(confidence),
    authenticityLabel: confidence,
...
export function normalizeCerulSearchResult({
  ...
  const authenticity = authenticityFromConfidence(confidence);
  ...
  const verdict = results.length
    ...
    : { status: "pending", verdictLabel: "Need Review", verdictTitle: "Need Review" };
